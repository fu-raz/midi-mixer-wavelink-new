...
module.exports = class Controller
{
    ...
    outputsChanged()
    {
        ...
        if (this.WaveLink.outputs)
        {
            ...
            $MM.getSettings().then((mmSettings) => {
                ...
                // I guess we need this for backwards compatibility
                let wavelinkOutputs = (this.WaveLink.outputs.hasOwnProperty('localMixer')) ? this.WaveLink.outputs.localMixer : this.WaveLink.outputs;

                // First set the amount of output devices in the settings tab
                $MM.setSettingsStatus('outputDevicesCount', wavelinkOutputs.length);
                ...
                for (let i = 0; i <= 9; i++)
                {
                    if (i < wavelinkOutputs.length)
                    {
                        let output = wavelinkOutputs[i];
                ...
                let wavelinkSelectedOutput = ( this.WaveLink.selectedOutput.hasOwnProperty('localMixer') ) ?  this.WaveLink.selectedOutput.localMixer :  this.WaveLink.selectedOutput;
    
                if (this.WaveLink.selectedLocalOutput) wavelinkSelectedOutput = this.WaveLink.selectedLocalOutput;

                if (wavelinkSelectedOutput)
                {
                    console.log('Selected output', wavelinkSelectedOutput);
                    console.log('Selected local output', this.WaveLink.selectedLocalOutput);
                    
                    if (this.outputDevices.has(wavelinkSelectedOutput) && this.WaveLink.outputs)
                    {
                        this.selectOutputDevice(wavelinkSelectedOutput);
                    }
                }
                ...
                if (this.outputRotation.size > 1)
                {
                    ...
                    if (this.outputRotation.has(wavelinkSelectedOutput))
                    {
                        // Because we just set this map, we can assume there are no active outputs and just activate the current one
                        this.outputRotation.get(wavelinkSelectedOutput).active = true;
                    }
    ...
    {
        console.log('selectOutputDevice', outputIdentifier);
