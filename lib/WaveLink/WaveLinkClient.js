...
module.exports = class WaveLinkClient extends AppClient {
                    ...
                    if (microphoneSettings.identifier == identifier) {
                        switch (property) {
                            ...
                            case kPropertyMicrophoneLowCutType:
                                ...
                                break;
                            case kPropertyMicrophoneGainLocked:
                                microphoneSettings.isGainLocked = value;
                                break;
                            case kPropertyMicrophoneMute:
                                microphoneSettings.isMicMuted = value;
                                break;
                            default:
                                break;
                        }
    ...
    async connectWithApp() {
        if (!this.isConnected && this.counterTries < this.maxTries) {
            console.log('Getting application info');
        ...
        this.call(kJSONPropertyGetApplicationInfo).then((result) => {
            if (result || result == undefined) {          
                ...
                if (result[kJSONKeyInterfaceRevision] >= kJSONPropertyInterfaceRevision) {
                    this.debug(`Minimum ${kJSONPropertyAppName} interface revision or above found.`);
                    this.debug(`Minumum interface revision for ${kJSONPropertyAppName} found: Current: ${result[kJSONKeyInterfaceRevision]}, Minimum: ${kJSONPropertyInterfaceRevision}`);
                    ...
                    this.setConnectionState(true);
                    this.isUpToDate = false;
                    this.debug(`Wrong interface revision for ${kJSONPropertyAppName}: Current ${result[kJSONKeyInterfaceRevision]}, Minimum: ${kJSONPropertyInterfaceRevision}`);
                    this.emitEvent(kPropertyUpdatePI);
                    this.emitEvent(kPropertyOutputChanged);
                    this.emitEvent(kJSONPropertyInputsChanged);
                }        
    ...
    setSelectedOutput(identifier) {
        ...
        let wavelinkOutputs = (this.outputs.hasOwnProperty('localMixer')) ? this.outputs.localMixer : this.outputs;

        const name = wavelinkOutputs.find( output => output.identifier == identifier)?.name;

        this.rpc.call(kJSONPropertySetSelectedOutput, {
            [kJSONKeyIdentifier]: identifier,
            [kJSONKeyMixerID]: kPropertyMixerIDLocal,
            [kJSONKeyName]: name
        });
