...
const EE = new EventEmitter();
 ...
 module.exports = class AppClient {
    ...
    connect() {
        ...
        console.log(`AppClient: Start connecting`);
    ...
    tryToConnect(port) {
        console.log(`AppClient: Trying to connect to port ${port}...`);
        ...
        websocket.onopen = () => {
            console.log(`AppClient: Connection established, connected to port ${port}.`);
        ...
        websocket.onclose = async () => {
            if (!this.tryReconnecting.get(port) || this.connectionTryCounters.get(port) > this.maxConnectionTries) {
                console.log(`AppClient: Connecting to port ${port} closed.`);
            }
        ...
        websocket.onmessage = (evt) => {
            if (!evt.data.includes('realTimeChanges')) {
                if (typeof evt.data === 'string') {
                    console.log("Incoming Message JSON", JSON.parse(evt.data));
                } else {
                    console.log("Incoming Message", typeof evt.data, evt.data);
                }
            }
    ...
    async reconnect(port) {
        ...
        if (this.tryReconnecting.get(port) && counter <= this.maxConnectionTries && this.websockets.get(port)?.readyState != 1) {
            //console.log(`AppClient: Reconnecting to port ${this.currentPort}, try no. ${this.connectionTryCounter}`);
            counter++;
            this.tryToConnect(port);
        } else {
            counter = 1;
